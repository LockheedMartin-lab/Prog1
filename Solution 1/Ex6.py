...
#%%# T6 A1 -> rocket  
m_start = 2.75e6 #start mass
R = 13.5e3 #fuel burn rate
Fs = 35.1e6 #Fschub
g = 9.81 #legendary g
c = 340 #Sound barrier
a_s = Fs/m_start - g #a(t)
T_S1 = 0.727 * m_start #
m_end = 0.05*T_S1 #end mass
ts = 0.05 #delta
m_nutz = m_start*(1-0.727) #payload

Zeit=[]               #time list / horizontal axis
Geschwindigkeit = []  #speed list / vertical axis
Schallpunkt = []      #sound barrier
t = 0
v = 0 
m = m_start
a = a_s
while m-m_nutz >= m_end: 
    a = Fs/m - g 
    m = m - R * ts
    v = v + ts * a 
    ...
    Zeit.append(t)
    t += 0.05
    print(f"t = {t:.2f} s, v = {v:.3f} m/s")
    if v >= 340:
        Schallpunkt.append(t)
      
        
print(f"Anfangsbeschleunigung: {a_s:.3f} m/s^2")
print(f"Endbeschleunigung: {a:.3f} m/s^2")
print(f"Schallgeschwindigkeit bei {Schallpunkt[0]:.3f} s")

import matplotlib.pyplot as plt

plt.plot(Zeit, Geschwindigkeit) #x/y lists
plt.hlines(y = 340, xmin = 50, xmax = 70, colors = "r", linestyle ="--", label = "Schallgeschwindigkeit") #red line
plt.legend(['Geschwindigkeit der Rakte', "Schallgeschwindigkeit"]) #naming
plt.grid(True)
plt.show()
